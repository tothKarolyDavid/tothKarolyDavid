...
**Professional & Clean.** Waving banner header, custom 2-col project cards (no plain pin cards), `tokyonight` theme for stats, focused socials.

Brand color: `#0078D4` (.NET Blue). Card label background: `#1a1b27`.

## Structure (top-to-bottom)

1. **Banner** — `capsule-render` SVG, name only
...
Edit URL fragment after `desc=` in the `capsule-render` `<img>`. URL-encode spaces as `%20`, dots/dashes left as-is. Verify URL returns 200 before committing.
...
|---|---|---|
| `capsule-render.vercel.app` | Header banner SVG | Params: type, color, text, desc |
